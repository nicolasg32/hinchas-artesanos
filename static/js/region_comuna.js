...
// Variables para elegir las Regiones y Comunas
let regionesYComunas = JSON.parse(json)["regiones"];
let regionSelector = document.getElementById("region");
let comunaSelector = document.getElementById("comuna");

// Agregamos todas las opciones de regiones
for(let i = 0; i < regionesYComunas.length; i++) {
    let option = document.createElement("option");
    option.text = regionesYComunas[i].nombre;
    option.value = i;
    option.id = option.value;
    regionSelector.add(option);
};

// Cada vez que se actualiza la seleccion de una region llamamos a la funcion actualizarComuna
regionSelector.addEventListener("change", actualizarComuna);

function actualizarComuna() {
    // Cada vez que actualizamos una region cambiamos la opcion a Seleccione una comuna
    comunaSelector.innerHTML = "<option value='no-selection-comuna'>Seleccione una comuna</option>";

    // Obtenemos la region que selecciono el usuario
    let regionSeleccionada = regionesYComunas[regionSelector.value];

    // Para cada comuna en la region seleccionada, la agregamos a comunaSelector
    // Rellenamos las comunas en caso de que el usuario selecciono una region valida
    if(regionSelector.value != "no-selection") {
